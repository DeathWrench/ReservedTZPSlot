...
namespace ReservedTZPSlot.Config
{
    public static class ConfigSettings
    {
        //public static ConfigEntry<bool> hideTZPMeshShoulder;
        ...
        public static void BindConfigSettings()
        {
            ...
            //hideTZPMeshShoulder = AddConfigEntry(Plugin.instance.Config.Bind("Client-side", "HideTZPOnShoulder", false, "Hides the TZP mesh while on your shoulder. Only applies in scenarios where you can view your player in third person."));
            overrideItemSlotPriority = AddConfigEntry(Plugin.instance.Config.Bind("Server-side", "TZPSlotPriorityOverride", -420, "[Host only] Manually set the priority for this item slot. Higher priority slots will come first in the reserved item slots, which will appear below the other slots. Negative priority items will appear on the left side of the screen, this is disabled in the core mod's config."));
            overridePurchasePrice = AddConfigEntry(Plugin.instance.Config.Bind("Server-side", "TZPSlotPriceOverride", 0, "[Host only] Manually set the price for this item in the store. Setting 0 will force this item to be unlocked immediately after the game starts."));
